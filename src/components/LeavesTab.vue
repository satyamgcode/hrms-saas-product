    ...
    <div class="mt-4 sm:mt-6 animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8">
      ...
      <div class="bg-white p-6 sm:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-purple to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-purple-100">
            <i class="mdi mdi-calendar-clock text-3xl"></i>
          </div>
          <div>
            <h2 class="text-2xl font-black text-gray-900 tracking-tight">Leave Management</h2>
            <p class="text-sm text-gray-500 font-medium mt-1">Manage your leaves, view entitlements, and check attendance metrics</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 bg-white p-6 sm:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div class="pt-6 border-t border-gray-100 flex items-center justify-between text-xs font-semibold text-gray-400">
            <span>Pending approvals:</span>
            <span class="px-2 py-0.5 rounded-full bg-orange-50 text-brand-orange border border-orange-100 font-bold">{{ pendingLeavesCount }} Requests</span>
          </div>
        </div>
              ...
              <tbody class="divide-y divide-gray-50 text-sm font-semibold">
                <tr v-for="leave in leaves" :key="leave.id" class="hover:bg-gray-50/50 transition-colors">
                  <td class="py-4 px-5">
                    ...
                    <p class="text-xs text-gray-400 font-medium mt-0.5 truncate max-w-[200px]" :title="leave.reason">{{ leave.reason }}</p>
                  </td>
                  <td class="py-4 px-5 text-gray-650">
                    <p class="text-xs font-bold">{{ new Date(leave.start_date).toLocaleDateString() }} - {{ new Date(leave.end_date).toLocaleDateString() }}</p>
                  </td>
                  ...
                  <td class="py-4 px-5">
                    ...
                    <p v-if="leave.comments" class="text-[10px] text-gray-450 italic mt-1 font-medium max-w-[150px] truncate" :title="leave.comments">
      ...
      <div v-if="showRequestModal" 
           class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
        <div class="bg-white rounded-[32px] w-full max-w-lg shadow-2xl border border-gray-100 p-6 sm:p-8 animate-in zoom-in-95 duration-200 relative">
          
          <button @click="showRequestModal = false" class="absolute top-6 right-6 p-2 rounded-xl text-gray-400 hover:text-gray-950 hover:bg-gray-100 transition-colors">
          ...
          <div class="space-y-5">
            <!-- Leave Type -->
            <div>
              <label class="block text-xs font-black text-gray-450 uppercase tracking-widest mb-2 ml-1">Leave Type</label>
              <div class="relative">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-black text-gray-450 uppercase tracking-widest mb-2 ml-1">Start Date</label>
              ...
              <div>
                <label class="block text-xs font-black text-gray-450 uppercase tracking-widest mb-2 ml-1">End Date</label>
            ...
            <!-- Reason -->
            <div>
              <label class="block text-xs font-black text-gray-450 uppercase tracking-widest mb-2 ml-1">Reason for Leave</label>
              <textarea v-model="leaveForm.reason" rows="3" placeholder="Explain the reasons for your leave application..."
